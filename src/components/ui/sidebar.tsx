  ...
  return (
    <Switch>
      <Match when={local.collapsible === "none"}>
        <div
          class={cn(
            "test w-(--sidebar-width) bg-sidebar text-sidebar-foreground flex h-full flex-col",
            local.class,
          )}
          {...others}
        >
      ...
      <Match when={isMobile()}>
        <Sheet open={openMobile()} onOpenChange={setOpenMobile} {...others}>
          <SheetContent
            ...
            data-mobile="true"
            class="w-(--sidebar-width) bg-sidebar text-sidebar-foreground p-0 [&>button]:hidden"
            style={{
